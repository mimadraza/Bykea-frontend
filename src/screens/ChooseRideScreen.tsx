...
const ChooseRideScreen: React.FC = () => {
  ...
  const [fareCount, setFareCount] = useState({ Motorbike: 146, Car: 512, RickShaw: 512 });
  const [selectedRide, setSelectedRide] = useState("Motorbike");
  const [customVisible, setCustomVisible] = useState(false);
  const [editingRide, setEditingRide] = useState<keyof typeof fareCount | null>(null);
    ...
    <View style={styles.container}>
      ...
      <View style={[
        ...
        <View style={styles.markerColumn}>
          ...
          <AccessibleText style={{ fontSize: 20, color: colors.text }}>➤</AccessibleText>
        </View>
      ...
      <View style={[styles.bottomSheet, { backgroundColor: colors.sheetBackground }]}>
            ...
            {rideOptions.map((ride) => (
              <TouchableOpacity
                ...
                style={[
                  styles.rideCard,
                  {
                    ...
                    borderColor: selectedRide === ride.name ? colors.primary : colors.border,
              ...
              >
                <View style={styles.rideLeft}>
                  <View style={[styles.iconPlaceholder, { backgroundColor: colors.inputBackground }]}>
                ...
                <FareCounter
                  ...
                  onIncrease={() => setFareCount({ ...fareCount, [ride.name]: fareCount[ride.name] + 1 })}
                  onDecrease={() => setFareCount({ ...fareCount, [ride.name]: Math.max(0, fareCount[ride.name] - 1) })}
                  onOpenCustom={() => openCustomFare(ride.name as keyof typeof fareCount)}
                />
              </TouchableOpacity>
            ))}
          </ScrollView>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("RideRequest", { rideType: selectedRide, fare: fareCount[selectedRide] })}
        >
        ...
        <View style={styles.popupOverlay}>
          <View style={[
            ...
            <View style={styles.popupButtons}>
              <TouchableOpacity onPress={() => setCustomVisible(false)} style={styles.cancelButton}>
                <AccessibleText style={[styles.cancelText, { color: colors.textSecondary }]}>{t("cancel_btn")}</AccessibleText>
              </TouchableOpacity>
              <TouchableOpacity onPress={applyCustomFare} style={[styles.okButton, { backgroundColor: colors.primary }]}>
...
const styles = StyleSheet.create({
  ...
  locationCard: {
    ...
    padding: 18, borderRadius: 22, flexDirection: "row", zIndex: 40,
  },
  ...
  plus: { fontSize: 22, fontWeight: "bold" },
  bottomSheet: {
    position: "absolute", bottom: 0, width: "100%",
    padding: 20, borderTopLeftRadius: 28, borderTopRightRadius: 28,
    zIndex: 50, paddingBottom: 30, maxHeight: "68%",
  },
  sheetHandle: { width: 60, height: 5, backgroundColor: "#555", borderRadius: 3, alignSelf: "center", marginBottom: 15 },
  ...
  findRideButton: { paddingVertical: 18, borderRadius: 14, marginTop: 10 },
  findRideText: { color: "#1A1A1A", fontSize: 18, fontWeight: "800", textAlign: "center" },
  popupOverlay: {
    position: "absolute", top: 0, left: 0, right: 0, bottom: 0,
    backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "center", alignItems: "center", zIndex: 500,
  },
  popupBox: { width: "75%", borderRadius: 16, padding: 20, alignItems: "center" },
  popupTitle: { fontSize: 18, fontWeight: "700", marginBottom: 12 },
  popupInputBox: { width: "100%", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6, marginBottom: 20 },
  popupInput: { fontSize: 16, paddingVertical: 6 },
  popupButtons: { width: "100%", flexDirection: "row", justifyContent: "flex-end", marginTop: 8 },
  cancelButton: { paddingHorizontal: 12, paddingVertical: 6, marginRight: 10 },
  cancelText: { fontSize: 16 },
  okButton: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10 },
